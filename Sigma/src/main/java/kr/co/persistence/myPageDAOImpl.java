...
public class myPageDAOImpl implements myPageDAO{
   ...
   public int buyCharge() {
      return session.selectOne(NS+".buyCharge");
   }

   @Override
   public void admincashcharge(int cash) {
      session.update(NS+".adminCashCharge", cash);
   }

   @Override
   public void seller(int cash, String writer) {
      Map<String, Object> map = new HashMap<String, Object>();
      map.put("cash", cash);
      map.put("writer", writer);
      session.update(NS+".sellermoney", map);
   }

   @Override
   public List<gameVO> compname(int gdnum) {
      return session.selectList(NS+".compname", gdnum);
   }
