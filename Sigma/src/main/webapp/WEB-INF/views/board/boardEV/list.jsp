	...
	<table class="table table-hover" style="color: white">
		...
		<tbody>
			<c:forEach items="${dbevTO.list}" var="vo">
				<tr>
					<td>${vo.num}</td>
					<td><a href="/board/main/event/eventRead?num=${vo.num}&curPage=${dbevTO.curPage}&perPage=${dbevTO.perPage}">${vo.title}</a></td>
					<td>${vo.writer}</td>
					<td>${vo.updatedate}</td>
					<td>${vo.viewcnt}</td>
				</tr>
		...
		<div class="row text-center" style="text-align: center">
				<ul class="pagination"> <!-- ul에 pagination 클래스를 주면 예쁘다 -->
					...
					<c:if test="${dbevTO.curPage>1}">
						<li><a
							href="/board/main/event/event?curPage=${dbevTO.curPage-1}&perPage=${dbevTO.perPage}">&laquo;</a></li>
					</c:if>
						<!-- 주소창에서 perPage값을 조절하면서 확인할것 -->

					<c:forEach begin="${dbevTO.bpn}" end="${dbevTO.spn}" var="idx">
						<li class="${dbevTO.curPage == idx?'active':''}"><a
							href="/board/main/event/event?curPage=${idx}&perPage=${dbevTO.perPage}">${idx}</a></li>
						<!-- li에 클래스를 active로 주면 현재 페이지에 색이 들어간다 -->
					</c:forEach>

					<c:if test="${dbevTO.curPage<dbevTO.totalPage}">
						<li><a
							href="/board/main/event/event?curPage=${dbevTO.curPage+1}&perPage=${dbevTO.perPage}">&raquo;</a></li>
					</c:if>
