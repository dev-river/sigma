...
<hr>
		...
		<div class="row text-center" style="text-align: center">
			<ul class="pagination">
				<c:if test="${vo.curPage>1}">
					<li><a
						href="/admin/main/adminlist?curPage=${vo.curPage-1}&perPage=${vo.perPage}&searchType=${vo.searchType}&keyword=${vo.keyword}">&laquo;</a></li>
				</c:if>

				<c:forEach begin="${vo.bpn}" end="${vo.spn}" var="idx">
					<li class="${vo.curPage == idx?'active':''}"><a
						href="/admin/main/adminlist?curPage=${idx}&perPage=${vo.perPage}&searchType=${vo.searchType}&keyword=${vo.keyword}">${idx}</a></li>
				</c:forEach>

				<c:if test="${vo.curPage<vo.totalPage}">
					<li><a
						href="/admin/main/adminlist?curPage=${vo.curPage+1}&perPage=${vo.perPage}&searchType=${vo.searchType}&keyword=${vo.keyword}">&raquo;</a></li>
				</c:if>
			</ul>
		</div>
	</div>
	...
	$(document).ready(function(){
		...
		$("#searchBtn").on("click", function(){
			...
			var url = "/admin/main/adminlist?searchType="+searchType+"&keyword="+keyword;
			window.open(url);
		});
