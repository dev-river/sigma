				...
				<div class="row text-center" style="text-align: center; position: relative; left: 40%; width: 500px;">
					<ul class="pagination">
						...
						<c:if test="${vo.curPage>1}">
							<li><a
								href="/recommand/main/dcgameRec?curPage=${vo.curPage-1}&perPage=${vo.perPage}&searchType=${vo.searchType}&keyword=${vo.keyword}">&laquo;</a></li>
						</c:if>
						<!-- 주소창에서 perPage값을 조절하면서 확인할것 -->

						<c:forEach begin="${vo.bpn}" end="${vo.spn}" var="idx">
							<li class="${vo.curPage == idx?'active':''}"><a
								href="/recommand/main/dcgameRec?curPage=${idx}&perPage=${vo.perPage}&searchType=${vo.searchType}&keyword=${vo.keyword}">${idx}</a></li>
							<!-- li에 클래스를 active로 주면 현재 페이지에 색이 들어간다 -->
						</c:forEach>

						<c:if test="${vo.curPage<vo.totalPage}">
							<li><a
								href="/recommand/main/dcgameRec?curPage=${vo.curPage+1}&perPage=${vo.perPage}&searchType=${vo.searchType}&keyword=${vo.keyword}">&raquo;</a></li>
						</c:if>
					</ul>
				</div>
			</div>
...
$(document).ready(function() {
   $("#searchBtn").on("click", function() {
		...
		var url = "/recommand/main/dcgameRec?curPage=1&perPage=${vo.perPage}&searchType="+searchType+"&keyword="+keyword;
		window.open(url);
	});
});
